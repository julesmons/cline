import type { DiagnosticsManager } from "@extension/integrations/diagnostics/diagnostics.manager";
...
import { createDirectoriesForFile } from "@common/utils/filesystem/fs";
...
export class DiffViewProvider {
  ...
  constructor(private cwd: string, private diagnosticsManager: DiagnosticsManager) {}
  ...
  private async openDiffEditor(): Promise<vscode.TextEditor> {
    if (this.relPath == null || this.editType == null) {
  ...
  async revertChanges(): Promise<void> {
    if ((this.relPath == null || this.relPath.length === 0) || !this.activeDiffEditor) {
      return;
    }
    const fileExists = this.editType === "modify";
  ...
  async saveChanges(): Promise<{
    ...
    if ((this.relPath == null || this.relPath.length === 0) || (this.newContent == null || this.newContent.length === 0) || !this.activeDiffEditor) {
      return {
    ...
    */
    ...
    const newProblems = this.diagnosticsManager.formatDiagnostics(
      this.diagnosticsManager.getNewDiagnostics(this.preDiagnostics, postDiagnostics),
      [
  ...
  async update(accumulatedContent: string, isFinal: boolean): Promise<void> {
    if ((this.relPath == null || this.relPath.length === 0) || !this.activeLineController || !this.fadedOverlayController) {
